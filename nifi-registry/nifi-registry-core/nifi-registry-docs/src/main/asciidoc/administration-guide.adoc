...
|`nifi.registry.cluster.node.identifier`|Optional human-readable label for this node.  Stored in the `CLUSTER_NODE`
table and included in log messages to make it easier to identify which node generated a log entry.
When blank, the JVM hostname is used.
